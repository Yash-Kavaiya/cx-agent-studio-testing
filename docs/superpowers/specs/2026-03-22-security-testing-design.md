...
3. Tests execute against CX agents via the existing CES `sessions.runSession` API
...
Create SecurityTestRun (state=pending)
        │
        ▼
Queue background task: run_security_test()
        │
        ▼
┌───────────────────────────────────────┐
│ Background Task                       │
│  1. Load dataset from HF (using token)│
│  2. Sample/shuffle if configured      │
│  3. For each batch of prompts:        │
│     a. Call CESClient.run_session()   │
│     b. Analyze response for bypass    │
│     c. Store SecurityTestResult       │
│     d. Update completed_prompts       │
│  4. Calculate final ASR               │
...
| hf_token_encrypted | String | AES-256 encrypted HuggingFace token |
...
| name | String | Display name (e.g., "JailbreakBench Run #3") |
...
|--------|------|-------------|
| id | UUID | Primary key |
| security_test_run_id | UUID | FK to security_test_runs |
...
- HF token encrypted using Fernet (AES-128-CBC)
...
POST /api/security-testing/runs
     ...
     Response: { "run_id": "uuid", "state": "pending" }
...
GET  /api/security-testing/runs/{run_id}/results
     Query:    ?filter=successful_attacks&page=1&per_page=50
...
| facebook/cyberseceval3-visual-prompt-injection | CyberSecEval Visual | 1,000 | Multimodal (image-based) |
...
```
backend/app/api/routes/security_testing.py  # API router
backend/app/api/routes/settings.py          # Settings router (new)
backend/app/models/security_testing.py      # SQLAlchemy models
backend/app/models/user_settings.py         # UserSettings model
backend/app/services/huggingface_service.py # HF dataset loading
backend/app/services/attack_detector.py     # Attack success detection
...
|--------|------|---------|-------------|
| sample_size | int | 100 | Number of prompts to test (0 = full dataset) |
| batch_size | int | 10 | Prompts per batch before progress update |
| timeout_per_prompt | int | 30 | Seconds to wait for agent response |
...
1. **Token Storage**: HF token encrypted at rest with Fernet
2. **Token Exposure**: Token never returned in API responses
3. **Rate Limiting**: Respect HF API rate limits (avoid ban)
